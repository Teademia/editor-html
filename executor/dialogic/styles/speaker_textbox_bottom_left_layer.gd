...
@export var box_size: Vector2 = Vector2(460, 130)
@export var box_distance: int = 20
@export var box_left_margin: int = 24

@export_group('Portrait')
@export_subgroup('Portrait')
@export var portrait_stretch_factor: float = 0.24
@export var portrait_position: LimitedAlignments = LimitedAlignments.LEFT
@export var portrait_bg_modulate: Color = Color(0, 0, 0, 0.45)
...
func _apply_export_overrides() -> void:
	...
	panel.size = box_size
	panel.position = Vector2(box_left_margin, -box_size.y - box_distance)
	...
	if not name_label_font.is_empty():
